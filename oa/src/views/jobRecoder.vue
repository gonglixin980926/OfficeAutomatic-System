...
<!--				<el-button type="primary" size="small" icon="el-icon-search" @click="getData">查询</el-button>-->
	...
	<el-table :data='tableData' style="width: 100%;">
		<el-table-column prop="userName" label="记录人"  sortable>
		</el-table-column>
		<el-table-column prop="rdnr" label="日志内容">
		</el-table-column>
		<el-table-column prop="rzTime" label="记录时间"  sortable>
		</el-table-column>
	</el-table>
...
export default {
	...
	methods: {
		...
		submit() {
			let tmpTime = new Date().toLocaleDateString();
			var user = sessionStorage.getItem('user');
			user = JSON.parse(user)
			var obj = {
				userId: user.userId,
				userName:user.userName,
				rzTime: util.formatDate.format(new Date(tmpTime), 'yyyy-MM-dd'),
				rdnr: this.add.rdnr
			}
			addLog(obj).then((res) => {
				this.$message({
