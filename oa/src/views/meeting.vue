<template>
	<section>
		...
		<div style="margin:10px" v-if="isManager()">
			<el-row :gutter="20">
				<el-col :span="8">
					<el-input type="number" v-model="hysbh"></el-input>
				</el-col>
				<el-col :span="2">
					<el-button type="primary" size="small" @click="addMeeting">新增会议室</el-button>
				</el-col>
			</el-row>
		</div>
		<!--列表-->
		<template>
			<el-table :data="users" highlight-current-row  @selection-change="handleSelectionChange" v-loading="loading" style="width: 100%;" max-height="550">
			...
			<el-button v-if="isManager()" style="margin: 10px" type="danger" @click="batchRemoveItem">批量删除</el-button>
		</template>
		...
		<el-dialog title="编辑会议室状态" :visible.sync="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="100px" ref="editForm">
				...
				<el-form-item label="开会内容" prop="bz">
...
<script>
	...
	import { getUserList,getHys,editHys,sendMessage,meetingSave,meetingBatchRemove } from '../api/api';
	export default {
		data() {
			return {
				selectList:[],
		...
		methods: {
			...
			addMeeting(){
				...
				meetingSave({"hysbh":this.hysbh,hyszt:"空闲"}).then(()=>{
					alert("添加成功");
					this.getTableData();
				})
			},
			...
			batchRemoveItem(){
				meetingBatchRemove(this.selectList).then(res=>{
					alert("删除成功")
					this.getTableData();
				})
			}
